...
namespace gl {
	...
	class Proxy {
	public:
		Proxy(T& host) noexcept : _host{ &host } {}
		template <typename S>
		Proxy<T> operator << (S data) noexcept(noexcept(*_host << data)) { return (_host && *_host << data) ? *this : Proxy<T>{}; }
	private:
		T* _host;
		constexpr Proxy() noexcept : _host{ nullptr } {}
	};
}
